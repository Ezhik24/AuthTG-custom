    ...
    public void onUpdateReceived(Update update) {
        if (update.hasMessage()) {
            if (update.getMessage().getText().toString().startsWith("/")) {
                ...
                if (update.getMessage().getText().toString().equals("/reg")) {
                    ...
                    sendMessage(update.getMessage().getChatId(), "[Бот] Напишите свой никнейм!");
                }
            ...
            if (nextStep.containsKey(update.getMessage().getChatId().toString())) {
                if (nextStep.get(update.getMessage().getChatId().toString()).equals("askplayername")) {
                    if (User.isNickname(update.getMessage().getText().toString())) {
                        ...
                        sendMessage(update.getMessage().getChatId(), "[Бот] Ожидайте... Выполняется проверка...");
                        if (User.getChatID(update.getMessage().getChatId().toString())) {
                            sendMessage(update.getMessage().getChatId(), "[Бот] Вы уже привязали аккаунт к телеграмму");
                        }else{
                            if (user != null) {
                                if (user.chatid.equals(update.getMessage().getChatId())) {
                                    this.sendMessage(update.getMessage().getChatId(), "[Бот] Аккаунт уже привязан к вам");
                                } else {
                                    this.sendMessage(update.getMessage().getChatId(), "[Бот] Аккаунт привяз к другому аккаунту");
                                }
                            } else {
        ...
        if (update.hasCallbackQuery()) {
            if (update.getCallbackQuery().getData().toString().startsWith("ys")) {
                ...
                sendMessage(update.getMessage().getChatId(), "[Бот] Успешный вход в аккаунт");
            }
            if (update.getCallbackQuery().getData().toString().startsWith("no")) {
                ...
                Handler.kick(playername, ChatColor.translateAlternateColorCodes('&', "Вы были кикнуты через Телеграмм"));
                this.deleteMessage(update.getCallbackQuery().getMessage());
            }
        }
    }
