...
public class CodeCMD implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        if (strings.length == 0) {
            commandSender.sendMessage(ChatColor.translateAlternateColorCodes('&', "&f&l[&b&lMT&f&l] &c&lНеверно введена комманда,используйте так: /code <код из телеграмма>"));
        } else {
            ...
            if (strings[0].equals(code.get(player.getUniqueId()))) {
                ...
                player.sendMessage(ChatColor.translateAlternateColorCodes('&', "&f&l[&b&lMT&f&l] &a&lУспешная привязка аккаунта"));
                try {
                    userconf.save(file);
                } catch (IOException e) {
            ...
            } else player.sendMessage("&f&l[&b&lMT&f&l] &c&lНеверный код");
        }
        return true;
    }
}
